...
class ImageExtractor:
    ...
    def __init__(
        ...
        # MAP ALREADY CALCULATED CHANGE AGENT POSITION BEFORE REACHING THIS LINE (inside self.sim)
        if not sim_provided:
            self.tdv_fp_ref_triples = self._preprocessing(
                self.sim, self.scene_filepaths, self.meters_per_pixel, self.ypos
            )
        else:
    ...
    def _preprocessing(self, sim, scene_filepaths, meters_per_pixel, ypos):
        tdv_fp_ref = []
        for filepath in scene_filepaths:
            ...
            while True:
                ...
                if np.abs(ref_point[1] - ypos) < 0.1:
                    break
    ...
    def _config_sim(self, scene_filepath, img_size):
        settings = {
            ...
            "default_agent": 0,
            "sensor_height": 0.88,  # 1.5,  # Height of sensors in meters
        ...
        for sensor_uuid, sensor_params in sensors.items():
            if settings[sensor_uuid]:
                ...
                sensor_spec.parameters["hfov"] = "79"
